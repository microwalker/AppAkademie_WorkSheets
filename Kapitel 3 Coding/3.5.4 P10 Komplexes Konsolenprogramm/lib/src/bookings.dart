...
final class cEntries extends Iterable {
  List<cEntry> entries = [];
  ...
  add(cEntry entry) => entries.add(entry);
...
abstract class cEntry {
  int? _id;
  DateTime? _datum;
  String? _text; 

  cEntry.empty(); 
  cEntry(this._datum, this._text) {this._id = this.hashCode;}

  get id => this._id;

  @override
  String toString() {
    return "[$_id] $_text am ${_datum.toString().split(" ")[0].split("-").reversed.join(".")}";
  }

  get value;
}

final class Soll extends cEntry {
  double? _soll;

  Soll(_datum, _text, this._soll) : super(_datum, _text);
...
final class Haben extends cEntry {
  double? _haben;

  Haben(_datum, _text, this._haben) : super(_datum, _text);
